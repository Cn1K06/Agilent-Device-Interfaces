...
public:
    ...
    : Node("agilent_3485a_node4"), ip_("172.17.2.21"), port_(1234), sockfd_(-1), measure_active_(false), should_exit_(false)
    {
        if (connectDevice()) {
...
private:
    ...
    std::vector<std::string> measurement_results_;
    ...
    void commandLoop() {
        ...
        while (rclcpp::ok() && !should_exit_) {
            ...
            if (command == "DC" || command == "AC" || command == "CURR_DC" || command == "CURR_AC" || command == "RES" || command == "FREQ") {
                ...
                RCLCPP_INFO(get_logger(), "Starting continuous %s measurement. Enter STOP to finish.", measurement_type_.c_str());

                input_thread_ = std::thread(&AgilentInterfaceNode::userInputListener, this);
                ...
                while (measure_active_ && rclcpp::ok()) {
                    ...
                    measurement_results_.push_back(result);

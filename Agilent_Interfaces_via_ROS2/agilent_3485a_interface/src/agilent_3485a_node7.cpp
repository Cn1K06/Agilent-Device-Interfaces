...
private:
    ...
    void performCalibration() {
        ...
        std::cout << "Do you want to perform calibration? (yes/no): ";
        std::getline(std::cin, user_input);
        std::transform(user_input.begin(), user_input.end(), user_input.begin(), ::tolower);
    
        if (user_input != "yes") return;
    
        std::string calib_type;
        std::cout << "Enter the type of calibration (ALL/DCV/AC/OHMS): ";
        std::getline(std::cin, calib_type);
        std::transform(calib_type.begin(), calib_type.end(), calib_type.begin(), ::toupper);
    
        if (calib_type == "DCV") {
            RCLCPP_INFO(get_logger(), "Performing DC voltage calibration (~3 minutes)...");
            sendCommand("ACAL 1");
            std::this_thread::sleep_for(std::chrono::minutes(3));
        } else if (calib_type == "AC") {
            RCLCPP_INFO(get_logger(), "Performing AC voltage calibration (~3 minutes)...");
            sendCommand("ACAL 2");
            std::this_thread::sleep_for(std::chrono::minutes(3));
        } else if (calib_type == "OHMS") {
            RCLCPP_INFO(get_logger(), "Performing resistance calibration (~12 minutes)...");
            sendCommand("ACAL 4");
            std::this_thread::sleep_for(std::chrono::minutes(12));
        } else if (calib_type == "ALL") {
            RCLCPP_INFO(get_logger(), "Performing full calibration (~16 minutes)...");
            sendCommand("ACAL 0");
            std::this_thread::sleep_for(std::chrono::minutes(16));
        } else {
            RCLCPP_WARN(get_logger(), "Invalid calibration type.");
            return;
        }
    }
    void perfomAutozero() {
        std::string auto_zero_input;
        std::cout << "Enable auto zero before measurement? (yes/no)\n"
                  << "(applies only to DC voltage, DC current, and resistance measurements): ";
        std::getline(std::cin, auto_zero_input);
        std::transform(auto_zero_input.begin(), auto_zero_input.end(), auto_zero_input.begin(), ::tolower);
        if (auto_zero_input == "yes") {
            enableAutoZero(true);
        } else {
    ...
    void enableAutoZero(bool enable) {
        if (enable) {
            sendCommand("AZERO ON");
            RCLCPP_INFO(get_logger(), "Auto zero enabled. Internal offset errors will be nulled .");
        } else {
            sendCommand("AZERO OFF");
            RCLCPP_INFO(get_logger(), "Auto zero disabled.");
        }
    }    
};

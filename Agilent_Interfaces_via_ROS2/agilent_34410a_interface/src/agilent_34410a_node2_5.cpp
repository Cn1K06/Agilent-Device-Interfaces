...
public:
  Agilent34410ANode() : Node("agilent_34410a_node") {
    ...
    // Learn the device's identity again
    identity_ = send_command("*IDN?");
    RCLCPP_INFO(this->get_logger(), "Device Identity: '%s'", identity_.c_str());
...
private:
  ...
  void reopen_device() {
    std::ofstream device_out(usbtmc_device, std::ios::out | std::ios::trunc);
    if (!device_out.is_open()) {
        RCLCPP_ERROR(this->get_logger(), "Failed to reopen USBTMC device for writing");
    }
    device_out.close();
    usleep(2000000); // 2 seconds wait
  }

  void reset_device_file() {
    std::ofstream device_out(usbtmc_device, std::ios::out | std::ios::trunc);
    if (!device_out.is_open()) {
      RCLCPP_ERROR(this->get_logger(), "Failed to reopen USBTMC device for writing");
    }
  ...
  void wait_for_completion() {
    usleep(2000000); // 2 seconds wait
    std::string response = send_command("*IDN?");
    if (response.empty() || response == "No response") {
      RCLCPP_ERROR(this->get_logger(), "Device not responding");
    } else {
      RCLCPP_INFO(this->get_logger(), "Device responsive: %s", response.c_str());
    }
  }

  void zero_calibration() {
    send_command("CAL:ZERO:ALL");

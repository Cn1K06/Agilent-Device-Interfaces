...
private:
  ...
  void set_square_wave(double freq, double amp, double duty_cycle) {
    std::ostringstream command;
    command << "APPL:SQU " << freq << "," << amp << "," << duty_cycle;
    send_command(command.str(), false);
    wait_for_operation_complete();
    check_device_status();
  }
  ...
  void set_ramp_wave(double freq, double amp, double symmetry) {
    std::ostringstream command;
    command << "APPL:RAMP " << freq << "," << amp << "," << symmetry;
    send_command(command.str(), false);
    wait_for_operation_complete();
    check_device_status();
  }

  void set_pulse_wave(double freq, double amp, double duty_cycle) {
    std::ostringstream command;
    command << "APPL:PULS " << freq << "," << amp << "," << duty_cycle;
    send_command(command.str(), false);
    wait_for_operation_complete();
    check_device_status();
  }
  ...
  void set_noise_wave(double amp) {
    send_command("APPL:NOIS DEF," + std::to_string(amp) + ",2", false);
    wait_for_operation_complete();
    check_device_status();
  }
  ...
  void configure_waveform(const std::string &waveform_type) {
    double freq, amp, duty_cycle, symmetry;
    ...
    if (waveform_type == "SIN") {
        ...
        std::cin >> duty_cycle;
        std::cin.ignore();  // Ignore the newline character after reading duty_cycle
        set_pulse_wave(freq, amp, duty_cycle);
